...
const getBulkFamilyData = async (req, res) => {
  try {
    ...
    // Define relation priority order
    const relationOrder = {
      'head': 1,
      'wife': 2,
      'husband': 2,
    ...
    const persons = await Person.aggregate([
      {
        $match: {
          family: { $in: familyIds.map(id => id.toString()) },
          status: 'active'
        }
      },
      {
        $addFields: {
          ...
          // Priority: customOrder if present, otherwise use relation order
          sortPriority: {
            $cond: {
              if: { $ne: ["$customOrder", null] },
              then: 0,  // customOrder items come first
              else: 1   // relation-based items come second
            }
          },
          // Map relation to numeric order
          relationOrder: {
            $switch: {
              branches: [
                { case: { $eq: ["$relation", "head"] }, then: 1 },
                { case: { $eq: ["$relation", "wife"] }, then: 2 },
                { case: { $eq: ["$relation", "husband"] }, then: 2 },
                { case: { $eq: ["$relation", "son"] }, then: 3 },
                { case: { $eq: ["$relation", "daughter"] }, then: 4 },
                { case: { $eq: ["$relation", "son in law"] }, then: 5 },
                { case: { $eq: ["$relation", "daughter in law"] }, then: 6 },
                { case: { $eq: ["$relation", "father"] }, then: 7 },
                { case: { $eq: ["$relation", "mother"] }, then: 8 },
                { case: { $eq: ["$relation", "brother"] }, then: 9 },
                { case: { $eq: ["$relation", "sister"] }, then: 10 },
                { case: { $eq: ["$relation", "grandson"] }, then: 11 },
                { case: { $eq: ["$relation", "granddaughter"] }, then: 12 }
              ],
              default: 999
            }
          },
          finalCustomOrder: {
            $ifNull: ["$customOrder", 999999]
          }
        }
      },
      {
        $sort: {
          family: 1,
          sortPriority: 1,          // First: items with customOrder
          finalCustomOrder: 1,       // Then: sort by customOrder value
          relationOrder: 1,          // Then: sort by relation priority
          name: 1                    // Finally: sort by name
        }
      }
    ]);
    ...
    const familyData = families.map(family => {
      ...
      const headPerson = familyPersons.find(person => 
      ...
      return {
        familyData: {
          _id: family._id,
          name: family.name,
          headname: headPerson ? headPerson.name : 'No head assigned',
          building: family.building,
          phone: family.phone,
        },
        parishInfo: family.parish,
