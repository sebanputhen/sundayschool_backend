...
async function getAllPersons(req, res) {
  try {
    ...
    const persons = await Person.find({ family: req.params.familyid, status: 'active' })
      .select("_id name baptismName relation gender education dob occupation status customOrder family")
      .sort({
        customOrder: 1, 
        relation: 1,    
        name: 1         
...
//       // Stage 1: Match persons early with index usage
//       {
//         $match: {
//           family: familyId,
//           status: 'active'
...
//       // Stage 2: Lookup family details (fewer documents to join)
//       {
//         $lookup: {
//           from: 'families',
//           localField: 'family',
...
//           as: 'familyDetails',
//           pipeline: [
//             { $project: { familyNumber: 1 } } // Only get required field
//           ]
//         }
//       },
      
//       // Stage 3: Single lookup for all transactions with conditional grouping
//       {
//         $lookup: {
//           from: 'transactions',
//           let: { 
...
//           },
//           pipeline: [
//             // Match all relevant transactions in one go
//             {
//               $match: {
//                 $expr: {
//                   $eq: ['$person', '$$personId']
...
//               }
//             },
//             // Group all calculations in a single stage
//             {
//               $group: {
//                 _id: null,
//                 // All-time totals
...
//           activeAmount: {
//             $ifNull: [
//               { $arrayElemAt: ['$transactionSummary.totalActiveAmount', 0] },
//               0
//             ]
//           },
//           transferredAmount: {
//             $ifNull: [
//               { $arrayElemAt: ['$transactionSummary.totalTransferredAmount', 0] },
//               0
//             ]
//           },
//           totalAmount: {
//             $add: [
//               { $ifNull: [{ $arrayElemAt: ['$transactionSummary.totalActiveAmount', 0] }, 0] },
//               { $ifNull: [{ $arrayElemAt: ['$transactionSummary.totalTransferredAmount', 0] }, 0] }
//             ]
//           },
//           originalAmount: {
//             $ifNull: [
//               { $arrayElemAt: ['$transactionSummary.totalOriginalAmount', 0] },
//               0
//             ]
//           },
//           currentYearActiveAmount: {
//             $ifNull: [
//               { $arrayElemAt: ['$transactionSummary.currentYearActiveAmount', 0] },
//               0
//             ]
//           },
//           currentYearTransferredAmount: {
//             $ifNull: [
//               { $arrayElemAt: ['$transactionSummary.currentYearTransferredAmount', 0] },
//               0
//             ]
//           },
//           currentYearAmount: {
//             $add: [
//               { $ifNull: [{ $arrayElemAt: ['$transactionSummary.currentYearActiveAmount', 0] }, 0] },
//               { $ifNull: [{ $arrayElemAt: ['$transactionSummary.currentYearTransferredAmount', 0] }, 0] }
//             ]
//           },
//           currentYearOriginalAmount: {
//             $ifNull: [
//               { $arrayElemAt: ['$transactionSummary.currentYearOriginalAmount', 0] },
//               0
//             ]
//           },
//           breakdown: {
...
const getFamilyDataWithTransactions = async (req, res) => {
  try {
    ...
    const prevStartDate = new Date(`${parsedYear - 1}-04-01`);
    const prevEndDate = new Date(`${parsedYear}-03-31`);
    ...
    const familyData = await Person.aggregate([
          ...
          pipeline: [
            ...
            // Group all calculations in a single stage
            {
              $group: {
                ...
                // Current year totals - ONLY ACTIVE STATUS
                currentYearActiveAmount: {
                  $sum: {
                    $cond: [
                      {
                ...
                // Previous year totals - ONLY ACTIVE STATUS
                prevYearActiveAmount: {
                  $sum: {
                    $cond: [
                      {
                        $and: [
                ...
                // Keep original calculations for transferred amounts if needed
                currentYearTransferredAmount: {
                  $sum: {
                    $cond: [
                      {
      ...
      // Stage 4: Project final result
      {
        $project: {
          ...
          // Current year amount - ONLY ACTIVE STATUS
          currentYearAmount: {
            $ifNull: [
              { $arrayElemAt: ['$transactionSummary.currentYearActiveAmount', 0] },
              0
          ...
          // Previous year amount - ONLY ACTIVE STATUS
          prevYearAmount: {
            $ifNull: [
              { $arrayElemAt: ['$transactionSummary.prevYearActiveAmount', 0] },
              0
            ]
          },
          // Keep other amounts if needed
          currentYearTransferredAmount: {
            $ifNull: [
              { $arrayElemAt: ['$transactionSummary.currentYearTransferredAmount', 0] },
              0
